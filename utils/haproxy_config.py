...
def render_haproxy_config():
    ...
    enabled_frontends = [
        ...
        if frontend.enabled and frontend.domain_name
    ]
    https_frontends = [frontend for frontend in enabled_frontends if frontend.use_ssl]
    http_frontends = [frontend for frontend in enabled_frontends if not frontend.use_ssl]
    http_redirects = [frontend for frontend in enabled_frontends if frontend.use_ssl and frontend.https_redirect]
    ...
    def register_backend(frontend):
        ...
        if frontend.acl and frontend.acl.backend_id:
            backend = backend_map.get(frontend.acl.backend_id)
            if backend:
                used_backend_set.add(backend.id)
...
def is_frontend_exist(frontend_name, domain_name, ignore_id=None):
...
def _append_frontend_rules(lines, frontends, backend_map, include_backend=True):
    ...
    for frontend in frontends:
        ...
        if redirect_rule:
            host_acl_name = f"redirect_host_{frontend.id}"
            root_acl_name = f"redirect_root_{frontend.id}"
